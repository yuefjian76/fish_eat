...
|-----------|-------|-------|
| **Build & Tests** | 5 | `npm test` 通过 981 个测试，0 失败 |
...
| **Balance Curve** | 5 | BalanceCurve 纯函数：成长表/敌人缩放/等级分布/刷怪权重/伤害上限，Lv1~11 食物链不变量测试 |
| **Boss System** | 5 | Lv5/8/11 三个 Boss（触发等级在等级上限内），配置驱动，进场动画，血量 UI，尺寸随玩家缩放 |
...
| **DEBUG_API** | 5 | `window.__DEBUG_API__` 17 methods + 16 E2E tests |
...
| `feature_list.json` | ✓ | Complete | 55 个功能状态，含证据 |
...
- **981 个单元测试**全部通过（56 suites）
- **63 个 E2E 测试**（12 个 spec 文件）覆盖核心游戏流程
